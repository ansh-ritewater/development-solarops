...
| 3 | **RESOLVED 14 Aug 2026** — added `cloudinaryThumb()` helper (`uploadToCloudinary.ts`), applied at all 11 confirmed thumbnail render sites (2 more than originally counted — `PhotoZone.tsx` had 2 photo-grid sites, not 1). Every `<a href={url}>` wrapper confirmed still points at the real full-size original, untouched. **Live-tested by Ansh.** | Largest real-world mobile/field win available | Low — display-only, no writes |
| 4 | **RESOLVED 14 Aug 2026** — `TaskCard` wrapped in `React.memo`; `handleCardClick` wrapped in `useCallback` with `[isAdmin, isViewOnly, currentUser?.role]`; all 7 real call sites (not 8, corrected count) now pass the same stable function reference instead of a fresh arrow each render. **Live-tested by Ansh.** | Stops up to 200 unnecessary re-renders per `appConfig` write | Low risk, conventional React, no business logic touched |
